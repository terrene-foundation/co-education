...
> **Implementation status (read first — this repo runs full enforcement).** Every step below is live: violation recording, the grace registration in Step 4 (via the engine CLI, announced by the session-start banner), detector escalation, the `regression_within_grace` emergency drop, and probe-confirmed cumulative downgrade are all mechanical, with `/cc-audit` step-15 adjudication wired and pinned by `.claude/audit-fixtures/posture-engine/run-fixtures.js`. The `## Trust Posture Wiring` authoring step (Steps 1–3, 5) remains a `/codify` authoring + gate-validation obligation as before.
