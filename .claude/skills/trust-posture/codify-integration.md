...
> **Implementation status (read first — this repo is hooks-only/advisory).** Violation recording, the grace registration in Step 4 (via the engine CLI, announced by the session-start banner), and detector escalation are live here. The mechanical posture-MOVING steps — the `regression_within_grace` emergency drop and probe-confirmed cumulative downgrade — require the `/cc-audit` step-15 adjudication machinery (and its `posture-engine` fixtures), which is NOT wired in this repo; until step 15 is landed (a human-gated upgrade to full enforcement) those events are recorded and surfaced but a posture change is a manual gate disposition. The `## Trust Posture Wiring` authoring step (Steps 1–3, 5) remains a `/codify` authoring + gate-validation obligation as before.
...
- an end-of-message self-confession scan → NO hook implements this (the substrate ships no stop-scan hook) — a rule needing it must say so and rely on gate review until a `/codify` proposal wires one
