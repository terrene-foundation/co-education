...
> **Implementation status (read first — this repo is hooks-only/advisory).** Grace registration via the posture CLI, the session-start grace banner, and detector escalation during the window are live here. The posture-MOVING step — `regression_within_grace` → instant one-level drop — and expiry-based graduation require the `/cc-audit` step-15 adjudication machinery and its `posture-engine` fixtures, which are NOT landed in this repo; until step 15 is wired (a human-gated upgrade to full enforcement) a grace regression is recorded and surfaced but the drop is a manual gate disposition. Two designed refinements remain UNBUILT anywhere: the `[ack: <rule_id>]` receipt token and the stale-record read-time check (the pre-rule "all-clear" skepticism injection). Wiring either requires a new `/codify` proposal.
