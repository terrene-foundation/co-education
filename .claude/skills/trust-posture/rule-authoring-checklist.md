...
> **Implementation status (read first — this repo is hooks-only/advisory).** The record + surface + deny-protect half each field relies on is live here: violation recording, grace registration, the `session-start.js` posture/grace banner, the deny + exit-2 path for the destructive/journal/bash guards (`validate-bash-command.js`, `destructive-op-guard.js`, `journal-write-guard.js`), and the hard Bash-write block on the posture-state files. The posture-MOVING half each field ultimately drives — `regression_within_grace` → instant one-level drop, and probe-confirmed cumulative downgrade — requires the `/cc-audit` step-15 adjudication machinery, which is NOT wired here; until step 15 is landed (a human-gated upgrade to full enforcement) those events are recorded and surfaced but a posture change is a manual gate disposition. Separately, ONE field has no mechanical enforcer anywhere: the `[ack: <rule_id>]` receipt token (and the `post-mortem` stop-scan it pairs with); wiring it requires a new `/codify` proposal.
...
- `post-mortem` — a detection only possible in the agent's final message; forensic only. NO mechanical detector exists (the substrate ships no stop-scan hook); this severity currently rests on gate review.
