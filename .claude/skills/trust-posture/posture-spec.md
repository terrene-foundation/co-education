...
> **Implementation status (read first — this repo runs full enforcement).** The enforcement engine is LIVE; the shapes below are the IMPLEMENTED on-disk shapes, pinned by `.claude/audit-fixtures/posture-engine/run-fixtures.js` (step-15 adjudication is wired). Two designed components still have NO mechanical implementation anywhere: a stop-scan self-confession pass (and with it the `acknowledgement_failure` event type) and the `[ack: <rule_id>]` receipt token. Either requires a new `/codify` proposal to wire.
