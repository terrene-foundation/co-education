...
> **Implementation status (read first — this repo is hooks-only/advisory).** The shapes below are the on-disk shapes the record + surface + deny-protect half writes here; the posture-MOVING half (probe-confirmed cumulative downgrade, emergency drops) requires the `/cc-audit` step-15 adjudication machinery and its `posture-engine` fixtures, which are NOT landed in this repo — a posture change is a manual gate disposition until step 15 is wired (a human-gated upgrade to full enforcement). Additionally, two designed components have NO mechanical implementation anywhere: a stop-scan self-confession pass (and with it the `acknowledgement_failure` event type) and the `[ack: <rule_id>]` receipt token. Either requires a new `/codify` proposal to wire.
