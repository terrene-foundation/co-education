...
## Enforcement Status (full enforcement — engine live, step-15 adjudication wired here)

This rule has two layers: a **behavioral contract** (what the agent MUST/MUST NOT do — live every session) and an **enforcement engine** (the mechanical layer that converts violations into posture changes — live). This repo runs **full enforcement**: violation recording, the session-start banner, the state-file write blocks, AND the `/cc-audit` step-15 adjudication loop that converts probe-confirmed verdicts into posture downgrades are all wired. This section states what the engine mechanically does so the rule neither over- nor under-claims (`rules/no-stubs.md` MUST §2).
...
Both paths are machine-applied in this repo: `detect-violations.js` records each hit, and the cumulative path (via the `adjudicate` CLI at `/cc-audit` step 15) and the emergency path apply the downgrade through the engine — step-15 adjudication is wired here (full enforcement).
