...
Origin: inbound sync from atelier (item-7 hook-cluster propagation, GH #15) — the L1–L5 graduated-autonomy ladder for the trust-posture hook substrate (`posture.json` + `detect-violations.js` + `permissions.deny`; phase names per CO v1.2). Companion to the `/autonomize` command — `/autonomize` adopts the autonomous posture for a session; THIS rule defines the trust ceiling that posture operates under and how that ceiling moves.
...
## Enforcement Status (hooks-only/advisory — recording + surfacing live; auto-downgrade deferred)

This rule has two layers: a **behavioral contract** (what the agent MUST/MUST NOT do — live every session) and an **enforcement engine** (the mechanical layer that converts violations into posture changes). This repo runs the engine in **hooks-only/advisory mode**: violation recording, the session-start banner, and the state-file write blocks are live; the auto-downgrade half (cumulative + emergency) requires the `/cc-audit` step-15 adjudication machinery, which is NOT wired here — recorded hits surface in the banner but do not move posture until step 15 is landed (a human-gated upgrade to full enforcement). This section states what the engine mechanically does so the rule neither over- nor under-claims (`rules/no-stubs.md` MUST §2).
...
- **Cumulative downgrade (where wired — full enforcement only):** in a repo that has wired `/cc-audit` step-15 adjudication, step 15 records each probe verdict via `node .claude/hooks/lib/posture.js adjudicate --ts <ts> --verdict confirmed|retired --probe <id> --by <gate>`; the engine annotates the entry with the structural `adjudicated:` marker and counts probe-CONFIRMED entries toward MUST §2's cumulative thresholds (one-level drop; entries consumed by a downgrade never count twice). This repo is hooks-only/advisory: step 15 is NOT wired, so this auto-downgrade path does not run — recorded hits stay advisory in the banner until step 15 is landed.
- **Emergency downgrade (where wired — full enforcement only):** in a full-enforcement repo a `regression_within_grace` recording drops one level instantly (MUST §3, the recording itself is wired in `detect-violations.js` here) and a confirmation carrying `--emergency destructive-op-unconfirmed|secret-leak|cross-repo-write-unauthorized` drops straight to `L1`. This repo records the `regression_within_grace` event and surfaces it, but the auto-downgrade requires step-15 wiring — until then the drop is a manual gate disposition, not a machine action.
...
**Mode note (this repo is hooks-only/advisory):** `detect-violations.js` records both paths' hits to `violations.jsonl` and the banner surfaces them, but the machine-applied downgrade — cumulative via the `adjudicate` CLI at `/cc-audit` step 15, and emergency — requires the step-15 adjudication machinery, which is NOT wired here. Until step 15 is landed (a human-gated upgrade to full enforcement), a downgrade is a manual gate disposition, not an automatic consequence.
...
When `/codify` lands a rule that addresses a detected or self-reported violation, that rule enters a grace window (default 7 days) recorded against `posture.json`. During grace, `detect-violations.js` runs the rule's detector in halt-and-report severity (not observer-only), and a violation of it records `regression_within_grace` → emergency downgrade (MUST §2) where step-15 adjudication is wired; in this hooks-only/advisory repo the event is recorded and surfaced but the auto-downgrade is deferred until step 15 lands. The grace window MUST NOT be treated as a soft warmup during which the new rule is advisory.
...
- `.claude/hooks/detect-violations.js` (agnostic detectors + grace escalation), `.claude/hooks/session-start.js` (posture banner + mirror pruning), `.claude/hooks/lib/posture.js` (the engine + gate CLI), `.claude/hooks/validate-bash-command.js` (Bash-path state-write deny), `.claude/audit-fixtures/validate-bash-command/` + `.claude/audit-fixtures/violation-patterns/` (the landed detector fixtures; the step-15 posture-engine fixtures are full-enforcement-only and not landed here), `.claude/learning/posture.json` + `violations.jsonl` (the state surface), `.claude/settings.json` `permissions.deny` (the no-self-modify enforcement).
