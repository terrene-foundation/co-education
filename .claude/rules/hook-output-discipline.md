...
Origin: inbound sync from atelier (item-7 hook-cluster propagation, GH #15) — the no-raw-exit(2) + structural-block-severity pattern for the trust-posture hook substrate (phase names per CO v1.2). Load-bearing here: the hook substrate (`.claude/hooks/`, `lib/runtime.js`, `detect-violations.js`) was landed this cycle.
