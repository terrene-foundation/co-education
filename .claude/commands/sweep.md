...
### Sweep 2: Open issues — this repository's tracker
...
- **Spec-deliverable mismatch** — spec promises X; deliverable has Y or nothing (`rules/specs-authority.md` MUST §6 — deviations require explicit acknowledgment)
...
Origin: adopted from atelier canonical sweep.md (GH atelier#15), hub-only sweeps (downstream sync currency, sibling-repo scans, pending-journal promotion — no `.pending` convention here) dropped for a standalone domain repo.
